...
namespace ProductSystem.Web.Controllers
{
    ...
    public class ManufacturersAdministrationController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,ManufactureName")] Manufacturer manufacturer)
        {
            if (db.Manufacturers.Any(x => x.ManufactureName == manufacturer.ManufactureName))
            {
                ModelState.AddModelError("ManufactureName", "There is already a manufacure with this name");
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,ManufactureName")] Manufacturer manufacturer)
        {
            if (db.Manufacturers.Any(x => x.ManufactureName == manufacturer.ManufactureName))
            {
                ModelState.AddModelError("ManufactureName", "There is already a manufacure with this name");
